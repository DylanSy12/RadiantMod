...
public class GodSmallStructureBlock extends Block {
			...
			for (int z=-2;z<=2;z++) {
				for (int y=0;y<=2;y++) {
					...
					if (block!=Main.godLuckyBlock.getDefaultState() && block!=Main.luckyBlock.getDefaultState() && block!=Main.mysticalLuckyBlock.getDefaultState()) {
						world.setBlockState(pos.add(x, y, z), Blocks.QUARTZ_BLOCK.getDefaultState());
						if (y!=0) {
							if (x>-2 && x<2 && z>-2 && z<2) {
								world.setBlockState(pos.add(x, y, z), Blocks.AIR.getDefaultState());
								if (x!=0 && z!=0 && y==1) {
									world.setBlockState(pos.add(x, y, z), Main.godLuckyBlock.getDefaultState());
								}
							}
						}
					}
				...
				if((x==-2 || x==2) && (z==-2 || z==2)) {
					world.setBlockState(pos.add(x, 3, z), Main.godBlock.getDefaultState());
				}
			}
		}
	}
