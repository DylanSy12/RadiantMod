...
public class CustomEvents {
	...
	public static List<BlockPos> destructionExplosions = new ArrayList<BlockPos>();
	...
	public static List<BlockPos> destructionExplosionsL = new ArrayList<BlockPos>();
...
//				System.out.println("bolts: "+bolts);
			...
			if (!destructionExplosions.isEmpty() && !explosionPower.isEmpty()) {
				BlockPos pos = destructionExplosions.get(0);
				float power = explosionPower.get(0);
				world.newExplosion(null, pos.getX(), pos.getY(), pos.getZ(), power, false, true);
				destructionExplosions.remove(pos);
				explosionPower.remove(power);
			}
			if (!destructionExplosionsL.isEmpty() && !explosionPowerL.isEmpty()) {
				BlockPos pos = destructionExplosionsL.get(0);
				float power = explosionPowerL.get(0);
				world.newExplosion(null, pos.getX(), pos.getY(), pos.getZ(), power, false, true);
				destructionExplosionsL.remove(pos);
