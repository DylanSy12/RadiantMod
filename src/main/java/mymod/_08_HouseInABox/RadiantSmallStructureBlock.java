...
public class RadiantSmallStructureBlock extends Block {
	...
	public RadiantSmallStructureBlock() {
		super(Material.ROCK);
					...
					if (block!=Main.godLuckyBlock.getDefaultState() && block!=Main.radiantLuckyBlock.getDefaultState() && block!=Main.mysticalLuckyBlock.getDefaultState()) {
						...
						if (y!=0) {
							if (x>-2 && x<2 && z>-2 && z<2) {
								...
								if (x!=0 && z!=0 && y==1) {
									world.setBlockState(pos.add(x, y, z), Main.radiantLuckyBlock.getDefaultState());
								}
							}
						}
					}
