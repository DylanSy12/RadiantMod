...
public class EntityGodClusterGrenade extends EntityThrowable {

	public EntityGodClusterGrenade(World worldIn, double x, double y, double z, double motX, double motY,
			double motZ) {
		super(worldIn, x, y, z);
		this.setVelocity(motX, motY, motZ);
	}
	
	public EntityGodClusterGrenade(World worldIn, EntityLivingBase throwerIn) {
		super(worldIn, throwerIn);
	}

	public EntityGodClusterGrenade(World worldIn) {
		super(worldIn);
	}
	...
	protected void onImpact(RayTraceResult result) {
		if (!this.world.isRemote) {
			...
			if(result.entityHit == null) {
				...
				while(number < 10) {
					world.spawnEntity(
						new EntityRadiantClusterGrenade(
