...
public class LegendMysticalSword1 extends ItemSword {
	private long time1;
	private long time2;
	private int range;
			...
			if (time2-time1>=this.cooldownTime || player.capabilities.isCreativeMode) {
				...
				if (pieces==4 || player.capabilities.isCreativeMode) {
					...
					if (entitylist.size()==1) {
						...
						CustomEvents.effectRadiusL.add(this.radius);
					}
					else {
				...
				if (player.capabilities.isCreativeMode && this.radius>3) this.radius-=3;
				else this.radius=3;
    ...
    {
		if (EnchantmentHelper.getEnchantmentLevel(Main.debuffingStrikeEnchant, stack)<3) {
			stack.addEnchantment(Main.debuffingStrikeEnchant, 3);
		}
		if (EnchantmentHelper.getEnchantmentLevel(Main.slowTouchEnchant, stack)<5) {
			stack.addEnchantment(Main.slowTouchEnchant, 5);
		}
		if (EnchantmentHelper.getEnchantmentLevel(Main.witheringTouchEnchant, stack)<4) {
			stack.addEnchantment(Main.witheringTouchEnchant, 4);
		}
		if (EnchantmentHelper.getEnchantmentLevel(Enchantments.BANE_OF_ARTHROPODS, stack)<10) {
    ...
    {
		if (this.radius<27 && this.boosted==false) {
			this.radius+=3;
			this.boosted=true;
