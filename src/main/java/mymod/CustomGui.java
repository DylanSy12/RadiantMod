...
//    protected final Map<ChatType, List<IChatListener>> chatListeners = Maps.<ChatType, List<IChatListener>>newHashMap();
    ...
    public CustomGui(Minecraft mcIn)
    {
        this.mc = mcIn;
...
//    }
    ...
    public static void renderCustomSwordHighlight(ItemStack item)
    {
        ...
        if (item.getItem() instanceof LegendMysticalSword || item.getItem() instanceof LegendRadiantSword || item.getItem() instanceof LegendGodSword || item.getItem() instanceof LegendDestructionSword)
        {
	        ...
	        int i = (scaledRes.getScaledWidth() - Minecraft.getMinecraft().ingameGUI.getFontRenderer().getStringWidth(s)) / 2;
	        int j = scaledRes.getScaledHeight() - 59;
            ...
            GlStateManager.enableBlend();
            GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
            Minecraft.getMinecraft().ingameGUI.getFontRenderer().drawStringWithShadow(s, (float)i, (float)j, 16777215 /*+ (k << 24)*/);
            GlStateManager.disableBlend();
            GlStateManager.popMatrix();
        }
